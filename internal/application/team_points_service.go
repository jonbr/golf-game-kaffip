...
func (s *TeamPointsService) SetHoleScore(ctx context.Context, gameID string, holeNumber int, req dto.SetHoleScoreRequest) (*domainGame.Game, error) {
	...
	result, err := teampoints.CalculateHoleResult(hole, inputs, g.Variant, g.PlayersByID(), g.TotalHoles())
	if err != nil {
		return nil, NewServiceError("invalid_hole_score", map[string]any{"underlying": err.Error()})
	}
